...
let notifTimeoutId = null; // глобальная переменная для хранения id таймера
...
function showNotification(message, type = 'error') {
    ...
    // Сбросить предыдущий таймер, если он есть
    if (notifTimeoutId) {
...
function addFirstContent(container) {
    ...
    document.getElementById('machines').addEventListener("keydown", function(e) {
    ...
    document.getElementById('diameters').addEventListener("keydown", function(e) {
        // Разрешаем только цифры, Backspace, Delete, стрелки, Tab
        if (
            e.key === "e" || e.key === "E" ||
            e.key === "+" || e.key === "-" ||
...
function addFourthContent(container) {
    ...
    document.getElementById('gen_table_fourth').addEventListener("click", function() {
        showNotification("Генерация данных...", 'success');
        clearResults();
        let contentTable = document.getElementById('machine_table_container');
        fillMachineTableRandom(contentTable);
    });
...
function fillMachineTableRandom(container) {
    ...
    for (let i = 1; i < rows.length; i++) { // пропускаем заголовок
        ...
        // Генерируем случайные значения, чтобы сумма была 100
        let remain = 100;
        let values = [];
        for (let k = 0; k < inputs.length - 1; k++) {
            // Случайное число от 0 до remain, округляем до целого
            let val = Math.floor(Math.random() * (remain + 1));
            values.push(val);
            remain -= val;
        }
        values.push(remain); // последний — остаток

        // Перемешаем значения для случайности распределения
        for (let k = values.length - 1; k > 0; k--) {
            const idx = Math.floor(Math.random() * (k + 1));
            [values[k], values[idx]] = [values[idx], values[k]];
        }
    ...
    showNotification("Таблица распределения заполнена случайными процентами", "success");
}
